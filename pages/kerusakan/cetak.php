...
<body>
  ...
  <center>
    ...
    <h3>LAPORAN KERUSAKAN INVENTARIS</h3>
  </center>
  ...
  // total barang
  $query = mysqli_query($conn, "select count(kode_kerusakan) as total from tbl_kerusakan_inventaris where jenis_inventaris='Barang'");
  $totalBarang = mysqli_fetch_assoc($query);
  // total alat
  $query = mysqli_query($conn, "select count(kode_kerusakan) as total from tbl_kerusakan_inventaris where jenis_inventaris='Peralatan & Mesin'");
  $totalAlat = mysqli_fetch_assoc($query);
  // total kendaraan
  $query = mysqli_query($conn, "select count(kode_kerusakan) as total from tbl_kerusakan_inventaris where jenis_inventaris='Kendaraan'");
  ...
  <table border="1" style="width: 100%;">
    ...
    $no = 1;
    $date = date("d-m-Y");
    $sql = mysqli_query($conn, "select * from tbl_kerusakan_inventaris where jenis_inventaris='Barang'");
    while ($data = mysqli_fetch_array($sql)) {
    ?>
      <tr>
        <td><?php echo $no++; ?></td>
        <td><?php echo $data['kode_kerusakan']; ?></td>
        <td><?php echo formatTanggal($data['tgl_kerusakan']); ?></td>
    ...
    ?>
    <tr>
      <td align='center' colspan="6">Jumlah Kerusakan Barang</td>
    ...
    $no = 1;
    $date = date("d-m-Y");
    $sql = mysqli_query($conn, "select * from tbl_kerusakan_inventaris where jenis_inventaris='Peralatan & Mesin'");
    while ($data = mysqli_fetch_array($sql)) {
    ?>
      <tr>
        <td><?php echo $no++; ?></td>
    ...
    ?>
    <tr>
      <td align='center' colspan="6">Jumlah Kerusakan Peralatan & Mesin</td>
    ...
    $no = 1;
    $date = date("d-m-Y");
    $sql = mysqli_query($conn, "select * from tbl_kerusakan_inventaris where jenis_inventaris='Kendaraan'");
    while ($data = mysqli_fetch_array($sql)) {
    ?>
      <tr>
        <td><?php echo $no++; ?></td>
    ...
    ?>
    <tr>
      <td align='center' colspan="6">Jumlah Kerusakan Kendaraan</td>
      <td align='center' colspan="1"><?= $totalKendaraan['total'] ?> mutasi</td>
    </tr>
    <tr>
      <td align='center' colspan="6">Jumlah Semua Kerusakan</td>
