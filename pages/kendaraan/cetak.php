...
<body>
  ...
  $query = mysqli_query($conn, "select sum(jumlah) as total from tbl_kendaraan");
  $totalSemua = mysqli_fetch_assoc($query);
  // total barang baru
  $query = mysqli_query($conn, "select sum(jumlah) as total from tbl_kendaraan where kondisi='baru'");
  $totalBaru = mysqli_fetch_assoc($query);
  // total barang bekas
  $query = mysqli_query($conn, "select sum(jumlah) as total from tbl_kendaraan where kondisi='bekas'");
  ...
  <table border="1" style="width: 100%;">
    ...
    $no = 1;
    $date = date("d-m-Y");
    $sql = mysqli_query($conn, "select * from tbl_kendaraan");
    ...
    ?>
    <tr>
      <td align='center' colspan="7">Jumlah Kendaraan Baru</td>
      <td align='center' colspan="2"><?= $totalBaru['total'] ?> unit</td>
    </tr>
    <tr>
      <td align='center' colspan="7">Jumlah Kendaraan Bekas</td>
      <td align='center' colspan="2"><?= $totalBekas['total'] ?> unit</td>
    </tr>
    <tr>
      <td align='center' colspan="7">Total Kendaraan</td>
